...
export function Particles({
  ...
  const fboType = useMemo(() => {
    const supportsFloat =
      gl.capabilities.isWebGL2 || gl.extensions.has("OES_texture_float");
    if (supportsFloat) return THREE.FloatType;
    const supportsHalfFloat =
      gl.capabilities.isWebGL2 || gl.extensions.has("OES_texture_half_float");
    return supportsHalfFloat ? THREE.HalfFloatType : THREE.UnsignedByteType;
  }, [gl]);
  ...
  useFrame((state, delta) => {
    ...
    state.gl.clear();
    state.gl.render(scene, camera as THREE.Camera);
  ...
  return (
    <>
      ...
      <points material={dofPointsMaterial as THREE.Material} {...props}>
        <bufferGeometry>
