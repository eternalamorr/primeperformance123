  ...
  constructor() {
    super({
      ...
      void main() {
        ...
        // Dynamic color based on position + time for a subtle red/blue shimmer
        float positionMix = (vWorldPosition.x + vWorldPosition.z) * 0.15 + 0.5;
        float timeMix = sin(uTime * 0.5 + (vWorldPosition.x + vWorldPosition.z) * 0.9) * 0.5 + 0.5;
        float colorMix = clamp(mix(positionMix, timeMix, 0.7), 0.0, 1.0);
        
        // Subtle, low-saturation red/blue palette
        vec3 warmColor = vec3(0.72, 0.18, 0.22);
        vec3 coolColor = vec3(0.18, 0.30, 0.75);
        vec3 particleColor = mix(warmColor, coolColor, colorMix) * 1.95;
        
        // Subtle sparkle highlight boost
        particleColor *= (0.9 + sparkleBrightness * 0.2);

        // Reduced opacity for subtler effect
        float finalAlpha = mix(alpha * 0.55, (sparkleBrightness - 0.9) * 0.45, uTransition);
